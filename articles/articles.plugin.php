	...
	class Articles extends Frontend {
		...
		public static function last($count=3, $what='last', $display=true){
			...
			if(count($records_sort)>0) {
				...
				if ($what == 'block')
					$view = View::factory('articles/views/frontend/block');
				else
		...
		public static function block($count=3) {
			Articles::last($count, 'block', true);
		}
		...
		public static function _shortcode($attributes) {
			...
			if (isset($list)) {
				switch ($list) {
					case 'last':
						return Articles::last($count, 'last', false);
					case 'views':
						return Articles::last($count, 'hits', false);
					case 'block':
						return Articles::last($count, 'block', false);
				}
			}
		}
